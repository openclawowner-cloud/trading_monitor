...
const BASE = API_ORIGIN ? `${API_ORIGIN}/api/bybit` : '/api/bybit';
...
export interface BybitAgent {
  id: string;
  mode: 'paper_local' | 'paper_exchange';
  enabled: boolean;
  extra?: Record<string, unknown>;
}
export interface BybitAgentDetail {
  agent: BybitAgent;
...
export type BybitCandleResponse = import('../types/api').CandleResponse & { venue?: string };

export const bybitClient = {
  getAgent: (id: string) => request<BybitAgentDetail>(`/agent/${encodeURIComponent(id)}`),
  ...
  getCandles: (symbol: string, interval: string, limit: number) => {
    ...
    return request<BybitCandleResponse>(`/candles?${q.toString()}`);
  }
};
