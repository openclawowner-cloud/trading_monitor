"""
Bybit testnet paper bot: CC_5M_BB_RSI-style ladder on spot quote-filtered markets.
"""
...
_bybit_dir = os.path.dirname(os.path.abspath(__file__))
_src_agents = os.path.normpath(os.path.join(_bybit_dir, "..", "agents"))
...
from bybit_public_client import (
    fetch_best_bid_ask,
...
def _agent_dir() -> str:
    ...
    if out:
        return out
    root = (os.environ.get("TELEMETRY_ROOT") or "").strip() or os.path.join(os.getcwd(), "trading-live-bybit")
    return os.path.join(root, "BYBIT_CC_5M_BB_RSI")


AGENT_ID = os.path.basename(os.path.normpath(_agent_dir())) or "BYBIT_CC_5M_BB_RSI"
...
def get_klines_df(symbol: str, interval: str = "5m", limit: int = 120) -> pd.DataFrame | None:
    rows = get_kline_rows(BYBIT_BASE, symbol, interval, limit)
    if not rows:
        return None
    records = []
...
def run_cycle():
    ...
    state["paused"] = paused
    # Emit an early heartbeat so supervisor/dashboard can mark agent as alive
    # even when the first market scan is still in progress.
    save_status(state, prices)

    ttl = float((os.environ.get("BYBIT_BOT_INSTRUMENTS_CACHE_SEC") or "3600").strip() or "3600")
    mx = int((os.environ.get("BYBIT_BOT_MAX_SCAN_MARKETS") or "40").strip() or "40")
    universe = get_cached_spot_quote_symbols(
        BYBIT_BASE, quote_asset=QUOTE_ASSET, ttl_sec=ttl, max_symbols=mx if mx > 0 else 0
    )
    if not universe:
    ...
    if open_symbols:
        ...
        if _consume_manual_sell_request() and pos["qty"] > 0:
            bid, _ = fetch_best_bid_ask(BYBIT_BASE, symbol)
            execute_trade(
                state,
                symbol,
                "sell",
                pos["qty"],
                float(bid) if bid else close_now,
                "manual_sell",
            )
            record_decision(
                state,
        ...
        if paused:
            record_decision(
                ...
                "hold",
                "paused",
                {"paused": True, "quoteAsset": QUOTE_ASSET},
            )
            save_state(state)
            save_status(state, prices)
            return
        for symbol in universe:
            ...
            ask = fetch_best_bid_ask(BYBIT_BASE, symbol)[1]
            # Execute only at/below trigger price; if orderbook cannot fill there, skip.
...
if __name__ == "__main__":
    _log(f"Starting {AGENT_ID} Bybit testnet 5m BB+RSI on */{QUOTE_ASSET}...")
    while True:
        try:
            run_cycle()
