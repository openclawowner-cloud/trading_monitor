...
import type { CryptoComAgentTelemetryRead, CryptoComRegistryAgent } from '../types';
import {
  deriveCryptoComAgentRuntimeStatus,
  getCryptoComAgentTelemetryBaseDir
} from './cryptoComTelemetry';
...
function getLastModifiedMs(agentId: string): number | null {
  const base = getCryptoComAgentTelemetryBaseDir(agentId.trim());
  let max = 0;
  let found = false;
...
export function mapCryptoComAgentToDashboardItem(
  agent: CryptoComRegistryAgent,
  ...
  const runtime = deriveCryptoComAgentRuntimeStatus(agentId, telemetry);
