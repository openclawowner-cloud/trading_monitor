...
import type { BybitAgentTelemetryRead, BybitRegistryAgent } from '../types';
import { deriveBybitAgentRuntimeStatus, getBybitAgentTelemetryBaseDir } from './bybitTelemetry';
...
function getLastModifiedMs(agentId: string): number | null {
  const base = getBybitAgentTelemetryBaseDir(agentId.trim());
  let max = 0;
  let found = false;
...
export function mapBybitAgentToDashboardItem(
  agent: BybitRegistryAgent,
  ...
  const runtime = deriveBybitAgentRuntimeStatus(agentId, telemetry);
  const status = archived ? 'archived' : enabled ? (runtime === 'unknown' ? 'offline' : runtime) : 'disabled';
  return {
    agentId,
    name,
