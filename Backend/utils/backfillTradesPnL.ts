/**
 * Fills missing per-trade `pnl` using the same weighted avg-cost model as the Cryptocoiner agent:
 * buy updates avg; sell realizes (price - avgCost) * qty - fee.
 * Only mutates copies; trades that already have `pnl` keep it and still update the running book.
 */
...
export function backfillTradesPnL(trades: unknown): unknown[] {
  if (!Array.isArray(trades) || trades.length === 0) {
    return Array.isArray(trades) ? [...trades] : [];
  }

  const copies = trades.map((t) =>
